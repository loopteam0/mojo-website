...
import { JobPost, PageResponse } from 'src/app/models/data.models';
import {
...
export class JobDetailsComponent {
  ...
  ngOnInit() {
    // Fetch the page data
    this.dataService.renderPage('Career Page').subscribe((response: PageResponse) => {
      this.PageData = response;
      console.log('PageData: ', this.PageData);

      // Assuming pageSection[3] contains the job posts data
      this.jobPositions = response.result.pageSection[3].jobPosts || [];
      console.log('Job Positions: ', this.jobPositions);

      // Update the selected job after data is fetched
      this.updateSelectedJob();
    });

    // Listen for changes in route parameters
    this.route.params.subscribe(() => {
      // Update the selected job when route parameters change
      this.updateSelectedJob();
    });
  }

  private updateSelectedJob() {
    // Extract jobTitle from the route parameters
    const jobTitle = this.route.snapshot.params['jobTitle'];
    console.log('Selected job title from route: ', jobTitle);

    // Find the job matching the jobTitle
    if (this.jobPositions.length > 0 && jobTitle) {
      this.selectedJob = this.jobPositions.find(
